...
// ISO/IEC 7812 card number length standards
...
export function validateRoute(req: Request, res: Response): void {
  ...
  // Ensure the input field is present and a string
  if (typeof rawNumber !== 'string') {
    logger.warn('Received request with non-string number field.', {
  ...
  try {
    ...
    // Determine card network (Visa, MasterCard, etc.)
    ...
    const maskedNumber = typeof rawNumber === 'string'
      ? maskCardNumber(rawNumber.replace(/[\s-]/g, ''))
      : 'N/A';
...
// Removes spaces/hyphens and validates length/format
function sanitizeInput(input: unknown): string {
  if (typeof input !== 'string') throw new Error('Missing or invalid "number" field');

  const cleaned = input.replace(/[\s-]/g, '');
  if (
    cleaned.length === 0 ||
    cleaned.length < MIN_CARD_NUMBER_LENGTH ||
    cleaned.length > MAX_CARD_NUMBER_LENGTH ||
    !/^\d+$/.test(cleaned)
  ) {
    if (cleaned.length === 0) throw new Error('Missing or invalid "number" field');
...
// Identifies the card scheme by prefix patterns
function getScheme(cardNumber: string): string {
  ...
  if ((prefix2 >= '51' && prefix2 <= '55') ||
      (prefix4 >= '2221' && prefix4 <= '2720')) return 'mastercard';
  if (prefix2 === '34' || prefix2 === '37') return 'amex';
  if (prefix4 === '6011' || prefix2 === '65' ||
      (prefix6 >= '622126' && prefix6 <= '622925')) return 'discover';
  return 'unknown';
}
